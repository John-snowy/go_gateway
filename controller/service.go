...
func (service *ServiceController) ServiceList(c *gin.Context) {
	...
	fmt.Println("This in serviceList: ",err)
	if err != nil {
	...
	for _, listItem := range list {
		...
		// １．http后缀接入 clusterIP + clusterPort + path
		// ２．http域名接入 domain
		// ３．tcp、grpc接入 clusterIP + servicePort
		serviceAddr := "unknow"
...
func (service *ServiceController) ServiceUpdateGRPC(c *gin.Context) {
	...
	grpcRule.HeaderTransfor = params.HeaderTransfer
	if err := grpcRule.Save(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 2006, err)
		return
