	...
	return func(srv interface{}, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
		if serviceDetail.AccessControl.ServiceFlowLimit > 0 {
			...
			if !serviceLimiter.Allow() {
				return errors.New(fmt.Sprintf("service flow limit %v", float64(serviceDetail.AccessControl.ServiceFlowLimit)))
			}
		}
		...
		if serviceDetail.AccessControl.ClientIPFlowLimit > 0 {
			...
			if !clientLimiter.Allow() {
				return errors.New(fmt.Sprintf("%v flow limit %v", clientIP, float64(serviceDetail.AccessControl.ServiceFlowLimit)))
			}
		}
